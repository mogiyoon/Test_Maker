...
export async function writeMakerSetting(makerSettingParm:string, toValue:string) {
...
export async function initiateMakerSettingStorage () {
  ...
  const makerSettingList = ['name', 'mean']
  ...
  if (initiateValue === makerSettingVersion) {
      ...
      for (let i = 0; i < makerSettingList.length; i++) {
        if (await AsyncStorage.getItem(makerSettingList[i]) === null) {
          await AsyncStorage.setItem(makerSettingList[i], makerSettingInitiateValue[i])
        }
      }
  }
}